...
Fully compatible with my other mods that replace the assembly file, just make sure you use this patch AFTER installing the customer stats mod or any other mods that fully replace the Assembly-CSharp-firstpass.dll.
...
## Notes
Tested on PC Building Simulator version 0.8.3.2. Might work with newer versions, but this is a binary patch - albeit a partial one - meaning if the code of the game changes in a future version chances are this patch will not work anymore. If you're unsure just make a backup copy of /PCBS_Data/Managed/Assembly-CSharp-firstpass.dll and then run the patch. If it says something like "no changes made" it is either already patched or not compatible with that version of the DLL.
